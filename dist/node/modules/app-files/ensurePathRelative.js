 ...
 */
export async function ensurePathRelative(http, appRootId, path, userId) {
    if (path.length === 0) {
        throw new Error('El path no puede estar vacío');
    }
    // Empezar desde el app root (nunca null)
    let currentParentId = appRootId;
    for (const segmentName of path) {
        ...
        const existingFolder = await findFolderByName(http, segmentName, currentParentId, // ⚠️ CONTRACTUAL: Nunca null
        userId);
        if (existingFolder) {
            currentParentId = existingFolder.id;
        }
        else {
            const newFolder = await createFolder(http, segmentName, currentParentId, // ⚠️ CONTRACTUAL: Nunca null
            userId);
 ...
 */
export async function resolvePathRelative(http, appRootId, path, userId) {
    if (path.length === 0) {
        return appRootId;
    }
    let currentParentId = appRootId;
 ...
 */
async function findFolderByName(http, name, parentId, // ⚠️ CONTRACTUAL: Nunca null en este contexto
userId) {
 ...
 */
async function createFolder(http, name, parentId, // ⚠️ CONTRACTUAL: Nunca null
userId) {
