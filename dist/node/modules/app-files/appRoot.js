 ...
 */
export function getAppRootFolderName(appId) {
 ...
 */
export async function getOrCreateAppRoot(http, appId, userId) {
 ...
 */
async function findAppRootFolder(http, name, userId) {
    ...
    const folder = rawItems.find((item) => item.type === 'folder' &&
        item.name === name &&
        item.userId === userId &&
 ...
 * En el futuro, esto debe usar POST /api/apps/:appId/root.
 */
async function createAppRootFolder(http, name, userId) {
