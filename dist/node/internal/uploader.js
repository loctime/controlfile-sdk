...
function filterUploadHeaders(headers) {
    const filtered = {};
...
async function uploadWithFetch({ url, file, method = 'PUT', headers = {}, onProgress, progressEnd = 90, }) {
    ...
    if (!response.ok) {
        throw new ControlFileError(`Upload failed with status ${response.status}`, 'UPLOAD_FAILED', response.status);
    }
    if (onProgress) {
        onProgress(progressEnd);
    }
}
function uploadWithXhr({ url, file, method = 'PUT', headers = {}, onProgress, progressStart = 10, progressEnd = 90, }) {
